      ...
      <div className="mx-auto flex h-16 w-full max-w-[1320px] items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center justify-center gap-2 md:flex">
          {customerLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-black hover:text-white"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1 rounded-full border border-black/15 px-3 py-1.5 text-xs font-semibold text-brand-black hover:bg-gray-100 sm:text-sm"
          >
