        ...
        {items.map((item, index) => (
          <div key={index} className="rounded border p-2">
            <div className="grid gap-2 md:grid-cols-6">
              <input className="rounded border p-2 md:col-span-2" placeholder="Description" value={item.description} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, description: event.target.value } : row))} />
              <input className="rounded border p-2" type="number" min="0.01" step="0.01" placeholder="Qty" value={item.qty} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, qty: event.target.value } : row))} />
              <input className="rounded border p-2" type="number" min="0" step="0.01" placeholder="Unit" value={item.unitPrice} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, unitPrice: event.target.value } : row))} />
              ...
              <input className="rounded border p-2" type="number" min="0" step="0.01" placeholder={item.discountType === 'percent' ? '%' : 'Amount'} value={item.discountValue} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, discountValue: event.target.value } : row))} />
            </div>
