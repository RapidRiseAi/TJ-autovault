        ...
        >
          <form
            ...
            onSubmit={(event) => {
              ...
              const selectedQuote = approvedQuotes.find(
    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-[0_14px_28px_rgba(17,17,17,0.08)]">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            ...
            {activeJob.started_at
              ...
              : 'just now'}{' '}
            • Updated {new Date(activeJob.last_updated_at).toLocaleString()}
          </p>
        </div>
        <span className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-xs font-semibold text-red-700">
          {formatJobCardStatus(activeJob.status)}
        </span>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        {activeJob.assignments.map((assignment) => (
          <span
            key={assignment.id}
            className="rounded-full border border-neutral-200 bg-neutral-50 px-2 py-1 text-xs"
          >
            {assignment.name}
          </span>
        ))}
      ...
      <Link
        ...
        className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-gray-600 hover:text-black"
      >
        View full internal timeline <ArrowRight className="h-3.5 w-3.5" />
      </Link>
      <Modal
      ...
      >
        ...
        <div className="mt-4 flex justify-end gap-2">
          <Button
            variant="secondary"
            disabled={isSaving || isRedirectingToInvoice || !canCloseNow}
            onClick={() => {
