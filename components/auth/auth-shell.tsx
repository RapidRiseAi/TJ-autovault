  ...
  return (
    <section className="relative rounded-3xl border border-white/10 bg-gradient-to-b from-zinc-900 via-zinc-950 to-black p-5 shadow-[0_16px_40px_rgba(0,0,0,0.35)]">
      ...
      <div className="relative mb-4 border-b border-white/15 pb-3">
        <span
          className="mb-2 inline-flex h-1.5 w-1.5 rounded-full bg-brand-red"
          aria-hidden
        />
      ...
      <div className="relative space-y-3">
        {items.map((item) => (
          <div
            key={item.title}
            className="flex gap-3 rounded-xl border border-white/10 bg-zinc-900/75 p-3"
          >
...
function TodayPanel() {
  ...
  return (
    <section className="relative rounded-3xl border border-white/10 bg-gradient-to-b from-zinc-900 via-zinc-950 to-black p-5 shadow-[0_16px_40px_rgba(0,0,0,0.35)]">
      ...
      <div className="relative mb-4 border-b border-white/15 pb-3">
        <span
          className="mb-2 inline-flex h-1.5 w-6 rounded-full bg-brand-red"
          aria-hidden
        />
      ...
      <ul className="relative space-y-2">
        {actions.map((item) => (
          <li
            key={item}
            className="flex items-center justify-between rounded-xl border border-white/10 bg-zinc-900/80 px-3 py-2 text-sm text-zinc-200"
          >
            <span>{item}</span>
            <svg
              viewBox="0 0 20 20"
      ...
      <div className="relative mt-4 rounded-xl border border-white/10 bg-zinc-800/90 px-3 py-3">
        ...
        <p className="mt-1 text-xs text-zinc-400">
          Need help? Contact the workshop.
        </p>
      </div>
...
function MobilePanel({
  ...
  return (
    <details className="rounded-2xl border border-white/15 bg-zinc-950/90 shadow-[0_10px_30px_rgba(0,0,0,0.3)] lg:hidden">
      <summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-zinc-100">
        {title}
      </summary>
...
export function AuthShell({ children }: { children: ReactNode }) {
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[#f6f6f7] text-brand-black">
      ...
      <div
        className="pointer-events-none absolute left-1/2 top-1/2 h-[560px] w-[560px] -translate-x-[68%] -translate-y-1/2 rounded-full border border-slate-500/10 opacity-[0.05] sm:opacity-[0.04]"
        aria-hidden
      >
        <span className="absolute inset-0 grid place-items-center text-[220px] font-black tracking-tight text-slate-800/70">
          TJ
        </span>
      </div>
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1340px] px-4 py-4 sm:px-6 lg:px-8 lg:py-8">
        <div className="grid items-start gap-4 lg:grid-cols-[minmax(220px,0.92fr)_minmax(0,600px)_minmax(220px,0.92fr)] lg:items-center">
          ...
          <section className="relative mx-auto w-full max-w-[600px]">
            <div
              className="pointer-events-none absolute inset-x-8 -bottom-8 -top-8 rounded-[40px] bg-red-500/8 blur-3xl"
              aria-hidden
            />
            {children}
          </section>
