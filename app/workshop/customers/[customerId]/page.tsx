...
export default async function WorkshopCustomerPage({ params }: { params: Promise<{ customerId: string }> }) {
  ...
  const [{ data: vehicles }, { count: unpaidInvoices }, { count: pendingQuotes }, { count: activeJobs }] = await Promise.all([
    ...
    supabase.from('service_jobs').select('id', { count: 'exact', head: true }).eq('customer_account_id', customerId).in('status', ['open', 'awaiting_approval', 'in_progress'])
  ]);
  ...
  return (
    <main className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-3">{[['Vehicles', vehicles?.length ?? 0], ['Open quotes', pendingQuotes ?? 0], ['Unpaid invoices', unpaidInvoices ?? 0], ['Active jobs', activeJobs ?? 0]].map(([l,v]) => <Card key={l as string}><p className="text-xs text-gray-500">{l}</p><p className="text-2xl font-bold">{v as number}</p></Card>)}</div>
      <Card>
