...
export async function GET(request: NextRequest) {
  ...
  const appliedCreditsFromInvoiceLines = new Map<string, number>();
  if (invoiceIds.length > 0) {
    const invoiceItems = await supabase
      .from('invoice_items')
      .select('invoice_id,description,line_total_cents,category')
      .in('invoice_id', invoiceIds);
    ...
    >) {
      ...
      const isCreditLine =
        String(item.category ?? '') === 'credit' ||
        /credit\s+note\s+.*applied|customer\s+account\s+credit\s+applied/i.test(
          String(item.description ?? '')
        );
      if (isCreditLine && lineTotalCents < 0) {
        const current = appliedCreditsFromInvoiceLines.get(invoiceId) ?? 0;
        appliedCreditsFromInvoiceLines.set(
          invoiceId,
          current + Math.abs(lineTotalCents)
        );
      }
    }
  }
