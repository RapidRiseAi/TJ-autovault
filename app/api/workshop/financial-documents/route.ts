...
export async function POST(request: NextRequest) {
  try {
    ...
    let customerEmail = customer.linked_email?.trim().toLowerCase() || null;

    if (!customerEmail && customer.auth_user_id) {
      try {
        const authUser = await admin.auth.admin.getUserById(customer.auth_user_id);
        customerEmail = authUser.data.user?.email?.trim().toLowerCase() || null;
      } catch (error) {
        console.error('Could not resolve customer auth email', error);
      }
    }
    ...
    if (customerEmail) {
      try {
        ...
        console.error('Financial document email failed', error);
      }
    }
