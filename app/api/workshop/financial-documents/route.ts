...
async function applyAvailableCustomerCredits(params: {
  ...
  for (const row of creditRows) {
    ...
    const applyScope = row.apply_scope === 'vehicle' ? 'vehicle' : 'customer';
    const rowVehicleId =
    ...
    const applyOnce = Boolean(row.apply_once);
    ...
    if (applyOnce && available > applyNow) {
      await params.supabase.from('customer_credit_ledger').insert({
        ...
        source_type: 'credit_application',
        source_id: params.invoiceId,
        description: `Unused carry-forward expired from ${(row.note_reference as string | null) ?? row.description ?? 'credit note'}`,
    ...
    applications.push({
      ...
      noteReference:
        typeof row.note_reference === 'string' ? row.note_reference : null,
      noteType:
