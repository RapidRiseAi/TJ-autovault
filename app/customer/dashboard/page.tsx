...
export default async function CustomerDashboardPage() {
  ...
  const { data: vehicles } = context.customerAccountIds.length
    ? await supabase
        .from('vehicles')
  ...
  return (
    <main className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href={customerVehicleNew()} className="rounded bg-brand-red px-3 py-2 text-sm font-medium text-white">
          Add vehicle
        </Link>
      </div>
      ...
      {(vehicles ?? []).length === 0 ? (
        <Card className="space-y-3">
          ...
          <Link href={customerVehicleNew()} className="inline-block rounded bg-brand-red px-4 py-2 text-sm font-medium text-white">
            Add vehicle
          </Link>
        </Card>
      ) : null}
      ...
      {(vehicles ?? []).map((vehicle) => (
        <Card key={vehicle.id} className="relative transition hover:border-brand-red/40 hover:shadow-md">
          <Link href={customerVehicle(vehicle.id)} className="absolute inset-0 rounded-lg" aria-label={`View vehicle details for ${vehicle.registration_number}`} />
          <div className="relative flex items-center justify-between gap-4">
            ...
            <div className="text-right text-sm">
              <Link href={customerVehicle(vehicle.id)} className="relative z-10 text-brand-red underline">
                View details
              </Link>
            </div>
          </div>
