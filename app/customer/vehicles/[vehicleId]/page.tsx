...
function VehicleAccessErrorPanel() { return <main className="space-y-4"><Card><h1 className="text-xl font-semibold">Vehicle unavailable</h1><p className="text-sm text-gray-700">Vehicle not found or you don&apos;t have access.</p><Link href={customerDashboard()} className="text-sm text-brand-red underline">Back to dashboard</Link></Card></main>; }
...
export default async function VehicleDetailPage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  const { data: vehicle } = await supabase.from('vehicles').select('id,registration_number,make,model,year,odometer_km,status,next_service_km,next_service_date').eq('id', vehicleId).eq('current_customer_account_id', customerAccountId).maybeSingle();
  if (!vehicle) return <VehicleAccessErrorPanel />;

  const [{ data: timeline }, { data: quotes }, { data: invoices }, { data: requests }, { data: recommendations }, { data: media }] = await Promise.all([
