...
export default async function WorkshopVehiclePage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  const [{ data: vehicle }, { data: jobs }, { data: recs }, { data: timeline }, { data: quotes }, { data: invoices }, { data: media }] = await Promise.all([
    ...
    supabase.from('vehicle_media').select('*').eq('vehicle_id', vehicleId).eq('workshop_account_id', profile.workshop_account_id).order('created_at', { ascending: false })
  ]);
  if (!vehicle) notFound();
  ...
  return <main className="space-y-4"><Card><h1 className="text-2xl font-bold">{vehicle.registration_number}</h1><p>{vehicle.make} {vehicle.model}</p><p className="text-xs">Status: {vehicle.status} · Odometer {vehicle.odometer_km ?? 'N/A'} km · Next service {vehicle.next_service_km ?? 'N/A'} km / {vehicle.next_service_date ?? 'N/A'}</p></Card><div className="grid gap-4 lg:grid-cols-3"><div className="lg:col-span-2 space-y-4"><Card><h2 className="font-semibold">Overview</h2><p className="text-sm">Open jobs: {jobs?.filter((j)=>j.status!=='completed'&&j.status!=='cancelled').length ?? 0}</p></Card><Card><h2 className="font-semibold">Timeline</h2>{(timelineRows ?? []).map((e)=><div key={e.id} className="my-2 border-l-2 pl-2"><div className="flex gap-2"><p>{e.title}</p><span className={`rounded border px-2 text-[10px] ${importanceBadgeClass(e.importance)}`}>{e.importance ?? 'info'}</span></div><p className="text-xs text-gray-500">{e.actorLabel} · {new Date(e.created_at).toLocaleString()}</p></div>)}</Card><Card><h2 className="font-semibold">Quotes</h2>{(quotes??[]).map((q)=><p key={q.id} className="text-sm">{q.status} · R{(q.total_cents/100).toFixed(2)}</p>)}</Card><Card><h2 className="font-semibold">Invoices</h2>{(invoices??[]).map((i)=><p key={i.id} className="text-sm">{i.status}/{i.payment_status} · R{(i.total_cents/100).toFixed(2)}</p>)}</Card><Card><h2 className="font-semibold">Recommendations</h2>{(recs??[]).map((r)=><p key={r.id} className="text-sm">{r.title} · {r.status} · {r.severity}</p>)}</Card><Card><UploadsSection vehicleId={vehicle.id} attachments={attachments} /></Card></div><Card><h2 className="mb-2 font-semibold">Quick actions</h2><VehicleWorkflowActions vehicleId={vehicle.id} invoices={(invoices??[]).map(i=>({id:i.id}))} jobs={(jobs??[]).map(j=>({id:j.id}))} /></Card></div></main>;
}
