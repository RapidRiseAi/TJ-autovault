create table if not exists public.timeline_deletion_requests (
  ...
  target_kind text not null check (target_kind in ('timeline','document')),
  target_id uuid not null,
  requested_by_profile_id uuid not null references public.profiles(id) on delete cascade,
  requested_by_role text not null check (requested_by_role in ('customer','workshop')),
  reason text,
...
alter table if exists public.timeline_deletion_requests enable row level security;

create unique index if not exists timeline_deletion_requests_pending_target_idx
  on public.timeline_deletion_requests(vehicle_id, target_kind, target_id)
  where status = 'pending';

create index if not exists timeline_deletion_requests_vehicle_status_idx
  on public.timeline_deletion_requests(vehicle_id, status, requested_at desc);
...
begin
  ...
  if v_customer_account_id is not null then
    ...
    where wu.profile_id = v_profile_id
  ) then
    v_requester_role := 'workshop';
  else
    raise exception 'Profile is not linked to a supported account';
  end if;
  ...
  if v_requester_role = 'customer' and not exists (
    select 1 from public.customer_users cu where cu.profile_id = v_profile_id and cu.customer_account_id = v_customer_account_id
  ) then
    raise exception 'You do not have access to this vehicle';
  end if;

  if v_requester_role = 'workshop' and not public.is_workshop_admin_for(v_workshop_account_id) then
    raise exception 'You do not have access to this vehicle';
  end if;
...
begin
  ...
  if exists (select 1 from public.customer_users cu where cu.profile_id = v_profile_id and cu.customer_account_id = v_request.customer_account_id) then
    v_actor_role := 'customer';
  elsif public.is_workshop_admin_for(v_request.workshop_account_id) then
...
drop policy if exists timeline_deletion_requests_select_customer on public.timeline_deletion_requests;
create policy timeline_deletion_requests_select_customer
...
using (
  exists (
    ...
    where cu.profile_id = auth.uid()
      and cu.customer_account_id = timeline_deletion_requests.customer_account_id
  )
);
...
revoke all on function public.request_timeline_item_deletion(text, uuid, text) from public;
grant execute on function public.request_timeline_item_deletion(text, uuid, text) to authenticated;

revoke all on function public.review_timeline_item_deletion(uuid, boolean, text) from public;
grant execute on function public.review_timeline_item_deletion(uuid, boolean, text) to authenticated;
